...
class ConfigManager
{
    ...
    public function updateConfig($data)
    {
        try {
            ...
            $keptIds = [];
            $insertStmt = $this->db->prepare("INSERT INTO accounts (access_key_id, access_key_secret, region_id, instance_id, max_traffic, schedule_enabled, start_time, stop_time, remark, account_type, traffic_used, instance_status, updated_at, last_keep_alive_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 0, 'Unknown', 0, 0)");
            $updateStmt = $this->db->prepare("UPDATE accounts SET access_key_secret = ?, region_id = ?, instance_id = ?, max_traffic = ?, schedule_enabled = ?, start_time = ?, stop_time = ?, remark = ?, account_type = ? WHERE id = ?");
            ...
            foreach ($newAccounts as $acc) {
                ...
                $params = [
                    ...
                    $acc['remark'] ?? '', $acc['account_type'] ?? 'domestic'
                ];
    ...
    private function reorderIds()
    {
        try {
            ...
            if (!empty($rows)) {
                ...
                $insertStmt = $this->db->prepare("INSERT INTO accounts (id, access_key_id, access_key_secret, region_id, instance_id, max_traffic, schedule_enabled, start_time, stop_time, remark, account_type, traffic_used, instance_status, updated_at, last_keep_alive_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
